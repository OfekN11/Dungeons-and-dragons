...
public class TileFactory {
    ...
    public Enemy produceEnemy(char tile, Coordinate position, MessageCallBack messageCallback,PlayerInRangeCallBack playerInRangeCallBack,TileInCoordinateProvider provider) {
        Enemy enemy = enemiesMap.get(tile).get();
        enemy.initialize(position,messageCallback,playerInRangeCallBack,provider);

        return enemy;
    }

    public Player producePlayer(int idx, InputProvider inputProvider) {
        Player player = playersList.get(idx).get();
        player.initialize(inputProvider);
        return player;
    }

    public Empty produceEmpty(Coordinate position) {
        Empty empty = new Empty();
        empty.initialize(position);
        return empty;
    }

    public Wall produceWall(Coordinate position) {
        Wall wall= new Wall();
